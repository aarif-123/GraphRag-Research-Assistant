        ...
        async with sse_client(mcp_url) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                response = await session.call_tool(
                    "search_arxiv", arguments={"query": query, "limit": limit}
                )
                ...
                for content in response.content:
                    if hasattr(content, "text") and content.text:
                        ...
                        try:
                            ...
                            log.warning(
                                f"Could not parse MCP text content as JSON: {content.text[:100]}"
                            )
                    ...
                    mapped_papers.append(
                        {
                            ...
                            "journal_ref": "",
                            "comment": "",
                            "categories": [paper.get("primary_category", "")]
                ...
                log.info(
                    f"Successfully retrieved and mapped {len(mapped_papers)} papers from remote ArXiv MCP server."
                )
